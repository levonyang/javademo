...
public class MyElasticsearchRestTemplate extends AbstractElasticsearchTemplate implements ElasticsearchOperations, EsClient<RestHighLevelClient>, ApplicationContextAware {

    private static final Logger logger = LoggerFactory.getLogger(MyElasticsearchRestTemplate.class);
    private static final int INDEX_MAX_RESULT_WINDOW = 1000;
    ...
    public Map<String, Object> getMapping(String indexName, String type) {
        ...
        try {
            ...
            mappings = getMappingsResponse.mappings().get(indexName).getSourceAsMap();
        } catch (Exception e) {
    ...
    public boolean typeExists(String index, String type) {
        if (type.equals("_doc")) {
            throw new UnsupportedOperationException("es 7.X 不支持其他类型: " + index + " type : " + type + " ");
        }
    ...
    public Map getSetting(String indexName) {
        ...
        try {
            GetSettingsRequest getSettingsRequest=new GetSettingsRequest(indexName);
            GetSettingsResponse response= client.indices().getSettings(getSettingsRequest,RequestOptions.DEFAULT);
            settings = convertSettingResponse(EntityUtils.toString(response.getIndexToSettings()), indexName);
    ...
    private Map<String, String> convertSettingResponse(String settingResponse, String indexName) {
        ...
        try {
            Settings settings = Settings.fromXContent(XContentType.JSON.xContent().createParser(NamedXContentRegistry.EMPTY, DeprecationHandler.THROW_UNSUPPORTED_OPERATION, settingResponse));
    ...
    private IndexRequest prepareIndex(IndexQuery query) {
        try {
            String indexName = StringUtils.isEmpty(query.getIndexName()) ? retrieveIndexNameFromPersistentEntity(query.getObject().getClass())[0] : query.getIndexName();
            String type = StringUtils.isEmpty(query.getType()) ? retrieveTypeFromPersistentEntity(query.getObject().getClass())[0] : query.getType();
            ...
            if (query.getObject() != null) {
                ...
                indexRequest.source(resultsMapper.getEntityMapper().mapToString(query.getObject()), Requests.INDEX_CONTENT_TYPE);
    ...
    private List<AliasMetaData> convertAliasResponse(String aliasResponse) {
        ...
        try {
            ...
            Map<String, AliasData> aliasData = mapper.readValue(mapper.writeValueAsString(node), new TypeReference<Map<String, AliasData>>() {});
            ...
            for (Map.Entry<String, AliasData> aliasentry : aliasIter) {
                ...
                aliasMetaDataList.add(AliasMetaData.newAliasMetaDataBuilder(aliasentry.getKey()).filter(data.getFilter()).routing(data.getRouting()).searchRouting(data.getSearch_routing()).indexRouting(data.getIndex_routing()).build());
            }
